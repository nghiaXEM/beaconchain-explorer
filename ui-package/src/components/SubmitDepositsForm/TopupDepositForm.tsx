...
const GWEI_PER_CUSTOME = BigInt(1e9);
...
const TopupDepositForm = (props: ITopupDepositFormProps): React.ReactElement => {
  ...
  const [topupAmount, setTopupAmount] = useState<number>(1); // UI input in CUSTOME (float)
  const [topupAmountGwei, setTopupAmountGwei] = useState<bigint>(BigInt(1e9)); // Actual amount in Gwei (BigInt)
  const [maxTopupAmount, setMaxTopupAmount] = useState<number>(0); // UI max in CUSTOME (float)
  const [walletBalance, setWalletBalance] = useState<bigint>(BigInt(100) * GWEI_PER_CUSTOME); // Wallet balance in Gwei
  ...
  useEffect(() => {
    if (selectedValidator) {
      ...
      // Calculate max topup amount in CUSTOME for UI (limited by wallet balance)
      const maxTopupEth = Number(remainingBalanceGwei / GWEI_PER_CUSTOME);
      
      // Set max topup amount for UI slider/input
      setMaxTopupAmount(maxTopupEth);
      
      // Reset topup amount to 1 CUSTOME when validator changes
      setTopupAmount(1);
      setTopupAmountGwei(GWEI_PER_CUSTOME); // 1 CUSTOME in Gwei
    }
  }, [selectedValidator, walletBalance, maxEffectiveBalance, maxEffectiveBalanceElectra]);

  const handleTopupSubmit = async () => {
    if (!selectedValidator || topupAmountGwei < GWEI_PER_CUSTOME) return;
      ...
      <div className="row mt-3">
        ...
        <div className="col-12">
          <div className="form-text">
            Select the validator you want to add more CUSTOME to. The validator must be active on the network.
          </div>
        </div>
        ...
        <>
          <div className="ms-2 mt-1">
            ...
            <div className="row">
              ...
              <div className="col-9 col-lg-10">
                {formatBalance(selectedValidator.balance, "CUSTOME")}
              </div>
            </div>
            <div className="row">
              <div className="col-3 col-lg-2">
          ...
          <div className="row mt-3">
            ...
            <div className="col-12">
              <div className="form-text">
                Enter an amount of at least 1 CUSTOME. Maximum amount is limited by your wallet balance and the validator's remaining space up to the effective balance limit.
              </div>
              ...
              <div className="row mt-3 withdrawal-details">
                ...
                <div className="col-7 col-md-6 col-lg-4">
                  {toReadableAmount(Number(getMaxEffectiveBalance()), 9, "CUSTOME", 0)}
                  {selectedValidator && selectedValidator.credtype !== "02" && (
                    <span 
                      ...
                      title={`This validator can be upgraded to the higher Electra limit of ${toReadableAmount(Number(maxEffectiveBalanceElectra), 9, "CUSTOME", 0)} by switching to a compounding validator (0x02 credentials) via self-consolidation.`}
                    >
              ...
              <div className="row mt-1 withdrawal-details">
                ...
                <div className="col-7 col-md-6 col-lg-4">
                  {toReadableAmount(Number(maxTopupAmount), 0, "CUSTOME", 3)}
                </div>
              </div>
              <div className="row mt-1 withdrawal-details">
                <div className="col-5 col-md-3 col-lg-2">
              ...
              <div className="mt-3">
                ...
                {topupAmount < 1 && (
                  <div className="text-danger mt-1">Amount must be at least 1 CUSTOME</div>
                )}
